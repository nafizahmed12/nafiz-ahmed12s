...
Revision ID: 0014_payment_state_machine
Revises: 0013_password_session_revocation
"""
...
revision: str = "0014_payment_state_machine"
down_revision: Union[str, Sequence[str], None] = "0013_password_session_revocation"
...
def upgrade() -> None:
    op.execute(
        ...
        BEGIN
            ...
            RAISE EXCEPTION
                'Invalid payment status transition: % -> %', OLD.status, NEW.status
...
def downgrade() -> None:
    op.execute("DROP TRIGGER IF EXISTS trg_payment_status_transition ON payments;")
