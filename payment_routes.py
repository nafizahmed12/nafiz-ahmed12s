...
def create_payment():
    ...
    with SessionLocal() as db:
        order = db.execute(
            text("""SELECT id, status, payment_status, currency, total_amount
        ...
        # Reuse an existing pending/initiated payment for the same provider.
        existing = db.execute(
            text("""SELECT id,order_id,provider,status,amount,currency,transaction_id,
        ...
        payment_id = db.execute(
            text("""INSERT INTO payments
                   (order_id,provider,transaction_id,status,amount,currency,
                    provider_reference,created_at,updated_at)
                   VALUES (:order_id,:provider,:transaction_id,:status,:amount,:currency,
                           :provider_reference,NOW(),NOW())
                   RETURNING id"""),
            {
                "order_id": order_id,
                "provider": provider,
        ...
        # COD/manual remains unpaid until fulfillment/payment collection confirms it.
        db.execute(
            text("""UPDATE commerce_orders
                    SET payment_status=:payment_status, updated_at=NOW()
                    WHERE id=:order_id"""),
            {"payment_status": "pending" if provider in {"cod", "manual"} else "initiated", "order_id": order_id},
        )
        db.commit()
    ...
    if provider in ONLINE_PROVIDERS:
        response["next_step"] = "Gateway initiation must be completed by the configured provider adapter."
    elif provider == "cod":
        response["next_step"] = "Collect payment on delivery; confirm through the protected webhook/admin flow."
...
def list_payments(order_id):
    ...
    with SessionLocal() as db:
        owns = db.execute(
            text("SELECT 1 FROM commerce_orders WHERE id=:order_id AND user_id=:user_id"),
            {"order_id": order_id, "user_id": user_id},
        ).scalar_one_or_none()
        if owns is None:
            return jsonify({"error": "Order not found."}), 404
        rows = db.execute(
            text("""SELECT id,order_id,provider,status,amount,currency,transaction_id,
                          provider_reference,created_at,updated_at
                   FROM payments WHERE order_id=:order_id ORDER BY id DESC"""),
            {"order_id": order_id},
        ).mappings().all()
...
def payment_webhook():
    ...
    with SessionLocal() as db:
        payment = db.execute(
            text("""SELECT id,order_id,status,amount,currency,transaction_id,provider_reference
                   FROM payments WHERE id=:payment_id FOR UPDATE"""),
            {"payment_id": payment_id},
        ).mappings().first()
        if payment is None:
            return jsonify({"error": "Payment not found."}), 404

        # Idempotent terminal updates: a duplicate paid/failed webhook is harmless.
        ...
        db.execute(
            text("""UPDATE commerce_orders
                    SET payment_status=:payment_status,
                        ...
                        updated_at=NOW()
                    WHERE id=:order_id"""),
            {"payment_status": order_payment_status, "order_id": payment["order_id"]},
        )
        db.commit()
