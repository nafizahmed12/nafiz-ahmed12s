...
def sslcommerz_initiate(order_id):
    ...
    with SessionLocal() as db:
        order = db.execute(text("""SELECT o.id,o.order_number,o.status,o.payment_status,o.currency,o.total_amount,u.username,u.email
            ...
            WHERE o.id=:order_id AND o.user_id=:user_id FOR UPDATE"""),
            {"order_id": order_id, "user_id": user_id}).mappings().first()
        if order is None:
            return jsonify({"error": "Order not found."}), 404
        if order["payment_status"] == "paid":
        ...
        if payment is None:
            tran_id = f"{order['order_number']}-{uuid4().hex[:6].upper()}"
            payment_id = db.execute(text("""INSERT INTO payments(order_id,provider,transaction_id,status,amount,currency,created_at,updated_at)
                ...
                {"order_id": order_id, "transaction_id": tran_id, "amount": order["total_amount"],
            ...
            db.execute(text("UPDATE commerce_orders SET payment_status='initiated',updated_at=NOW() WHERE id=:order_id"),
                       {"order_id": order_id})
            db.commit()
            payment = {"id": payment_id, "transaction_id": tran_id, "provider_reference": None,
                       "status": "initiated", "amount": order["total_amount"], "currency": order["currency"]}
        elif payment["status"] == "paid":
            return jsonify({"error": "Order is already paid."}), 409
        else:
            tran_id = str(payment["transaction_id"] or f"{order['order_number']}-{uuid4().hex[:6].upper()}")
        ...
        body = {
            ...
            "success_url": _callback_url("success"), "fail_url": _callback_url("fail"),
            "cancel_url": _callback_url("cancel"), "ipn_url": _callback_url("ipn"),
            "cus_name": order["username"], "cus_email": order["email"],
            "cus_add1": "Bangladesh", "cus_city": "Dhaka", "cus_state": "Dhaka", "cus_postcode": "1000",
            "cus_country": "Bangladesh", "shipping_method": "NO", "product_name": "Nafiz Commerce Order",
        ...
        try:
            ...
            logger.error("SSLCommerz HTTP error order_id=%s payment_id=%s tran_id=%s status=%s reason=%s body_preview=%r",
            ...
            logger.exception("SSLCommerz initiate failed order_id=%s payment_id=%s tran_id=%s sandbox=%s",
        ...
        if result.get("status") != "SUCCESS" or not result.get("GatewayPageURL"):
            logger.error("SSLCommerz rejected payment order_id=%s payment_id=%s tran_id=%s provider_status=%r failedreason=%r",
    ...
    return jsonify({"payment_id": payment["id"], "status": "initiated",
                    "gateway_page_url": result["GatewayPageURL"], "session_key": result.get("sessionkey")})
...
def _ssl_callback(status):
    ...
    with SessionLocal() as db:
        ...
        if status == "success":
            try:
                validation = _ssl_validate(val_id=val_id, tran_id=tran_id)
            except HTTPError as exc:
                error_body = exc.read().decode("utf-8", errors="replace")[:1000]
                logger.error("SSLCommerz validation HTTP error tran_id=%s val_id=%s status=%s reason=%s body_preview=%r",
                             tran_id, val_id, exc.code, exc.reason, error_body)
                db.rollback()
                return jsonify({"error": "Could not validate SSLCommerz transaction. Check Render logs for details."}), 502
            except Exception:
                logger.exception("SSLCommerz validation failed tran_id=%s val_id=%s", tran_id, val_id)
                db.rollback()
                return jsonify({"error": "Could not validate SSLCommerz transaction. Check Render logs for details."}), 502

            valid = isinstance(validation, dict) and validation.get("status") in {"VALID", "VALIDATED"}
            amount_ok = valid and _money(validation.get("amount")) == _money(payment["amount"])
            returned_currency = str(validation.get("currency") or validation.get("currency_type") or "").upper()
            expected_currency = str(payment["currency"] or "BDT").upper()
            currency_ok = valid and returned_currency == expected_currency
            tran_ok = valid and str(validation.get("tran_id", tran_id)) == tran_id
            if not (valid and amount_ok and currency_ok and tran_ok):
                logger.error("SSLCommerz payment validation failed tran_id=%s val_id=%s validation_status=%r amount=%r expected_amount=%s currency=%r expected_currency=%s tran_ok=%s",
                             tran_id, val_id, validation.get("status") if isinstance(validation, dict) else None,
                             validation.get("amount") if isinstance(validation, dict) else None, _money(payment["amount"]),
                             validation.get("currency") if isinstance(validation, dict) else None, expected_currency, tran_ok)
                db.rollback()
...
def sslcommerz_ipn():
    ...
    with SessionLocal() as db:
        ...
        if gateway_status in {"VALID", "VALIDATED"} or val_id:
            try:
                validation = _ssl_validate(val_id=val_id, tran_id=tran_id)
            except Exception:
                logger.exception("SSLCommerz IPN validation failed tran_id=%s val_id=%s", tran_id, val_id)
                db.rollback()
                return jsonify({"error": "Could not validate SSLCommerz transaction."}), 502
            valid = isinstance(validation, dict) and validation.get("status") in {"VALID", "VALIDATED"}
            amount_ok = valid and _money(validation.get("amount")) == _money(payment["amount"])
            returned_currency = str(validation.get("currency") or validation.get("currency_type") or "").upper()
            currency_ok = valid and returned_currency == str(payment["currency"] or "BDT").upper()
            tran_ok = valid and str(validation.get("tran_id", tran_id)) == tran_id
            ...
            return jsonify({"error": "Payment validation failed."}), 400
        db.commit()
        return jsonify({"ok": True, "status": gateway_status or "received"})
...
def payment_result(result):
    if result not in {"success", "fail", "cancel"}:
        return jsonify({"error": "Invalid payment result."}), 404
