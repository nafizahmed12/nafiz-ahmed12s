...
def product_detail(product_id):
    with SessionLocal() as db:
        ...
        if row is None: return jsonify({"error": "Product not found."}), 404
...
def _get_or_create_cart(db,user_id):
    ...
    if cart_id is not None:return cart_id
    try:return db.execute(text("INSERT INTO carts (user_id,created_at,updated_at) VALUES (:user_id,NOW(),NOW()) RETURNING id"),{"user_id":user_id}).scalar_one()
    except IntegrityError:
        db.rollback();return db.execute(text("SELECT id FROM carts WHERE user_id=:user_id"),{"user_id":user_id}).scalar_one()
...
def _cart_payload(db,user_id):
    ...
    for row in rows:
        unit=Decimal(str(row["unit_price"] or 0));line=unit*row["quantity"];subtotal+=line
...
def get_cart():
    ...
    with SessionLocal() as db: payload=_cart_payload(db,user_id);db.commit()
...
def add_cart_item():
    ...
    try:product_id=int(body.get("product_id"));quantity=int(body.get("quantity",1))
    except (TypeError,ValueError):return jsonify({"error":"product_id and quantity must be integers."}),400
    if quantity<1 or quantity>100:return jsonify({"error":"Quantity must be between 1 and 100."}),400
    with SessionLocal() as db:
        ...
        if product["product_type"]!="digital" and product["stock_quantity"]<quantity:return jsonify({"error":"Insufficient stock."}),409
        cart_id=_get_or_create_cart(db,user_id)
        ...
        if product["product_type"]!="digital" and new_quantity>product["stock_quantity"]:return jsonify({"error":"Requested quantity exceeds available stock."}),409
        if existing is None:db.execute(text("INSERT INTO cart_items (cart_id,product_id,quantity,created_at) VALUES (:cart_id,:product_id,:quantity,NOW())"),{"cart_id":cart_id,"product_id":product_id,"quantity":new_quantity})
        else:db.execute(text("UPDATE cart_items SET quantity=:quantity WHERE cart_id=:cart_id AND product_id=:product_id"),{"cart_id":cart_id,"product_id":product_id,"quantity":new_quantity})
        db.execute(text("UPDATE carts SET updated_at=NOW() WHERE id=:cart_id"),{"cart_id":cart_id});payload=_cart_payload(db,user_id);db.commit()
...
def update_cart_item(cart_item_id):
    ...
    except (TypeError,ValueError):return jsonify({"error":"quantity must be an integer."}),400
    if quantity<1 or quantity>100:return jsonify({"error":"Quantity must be between 1 and 100."}),400
    with SessionLocal() as db:
        ...
        if row is None:return jsonify({"error":"Cart item not found."}),404
        if row["product_type"]!="digital" and quantity>row["stock_quantity"]:return jsonify({"error":"Requested quantity exceeds available stock."}),409
        db.execute(text("UPDATE cart_items SET quantity=:quantity WHERE id=:item_id"),{"quantity":quantity,"item_id":cart_item_id});db.execute(text("UPDATE carts SET updated_at=NOW() WHERE id=(SELECT cart_id FROM cart_items WHERE id=:item_id)"),{"item_id":cart_item_id});payload=_cart_payload(db,user_id);db.commit()
    return jsonify(payload)
...
def delete_cart_item(cart_item_id):
    ...
    with SessionLocal() as db:
        ...
        if result.rowcount==0:return jsonify({"error":"Cart item not found."}),404
        db.execute(text("UPDATE carts SET updated_at=NOW() WHERE user_id=:user_id"),{"user_id":user_id});payload=_cart_payload(db,user_id);db.commit()
    return jsonify(payload)
...
def create_checkout():
    ...
    with SessionLocal() as db:
        ...
        for row in rows:
            if row["product_type"]!="digital" and row["quantity"]>row["stock_quantity"]:return jsonify({"error":"Stock changed. Please review your cart.","product_id":row["product_id"]}),409
...
def _order_number():return "NA-"+uuid4().hex[:20].upper()
...
def place_order(checkout_id):
    ...
    with SessionLocal() as db:
        ...
        for item in items:
            ...
            if item["product_type"]!="digital":db.execute(text("UPDATE products SET stock_quantity=stock_quantity-:quantity WHERE id=:product_id"),{"quantity":item["quantity"],"product_id":item["product_id"]})
        db.execute(text("UPDATE checkouts SET status='converted',updated_at=NOW() WHERE id=:checkout_id"),{"checkout_id":checkout_id});db.execute(text("DELETE FROM cart_items WHERE cart_id=(SELECT id FROM carts WHERE user_id=:user_id)"),{"user_id":user_id});db.execute(text("UPDATE carts SET updated_at=NOW() WHERE user_id=:user_id"),{"user_id":user_id});db.commit()
...
def register_commerce_routes(app):app.register_blueprint(commerce_bp)
